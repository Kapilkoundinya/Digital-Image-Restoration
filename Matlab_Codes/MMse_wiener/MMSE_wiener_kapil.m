...
sf=fft2(rf); %powerspectral density of input
...
for k1=1:M
for k2=1:N
Fcap(k1,k2) = ( conj( H(k1,k2) ) / ( pow2( abs( H(k1,k2) ) )+ (  sn(k1,k2) / sf(k1,k2) )  )  )* G(k1,k2); % restored image in spatial domain
end
end
...
%SNR calculation
SNR = norm(Fcap,2)/norm((double(F)-Fcap),2)
